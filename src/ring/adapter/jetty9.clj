...
(defn- http2-alpn-connection-factory [& args]
  ;; load http2 module dynamically
  (let [http2-alpn-connector-factory-ctor @(requiring-resolve 'ring.adapter.jetty9.http2/http2-alpn-connection-factory)]
    (apply http2-alpn-connector-factory-ctor args)))

(defn- http2-clear-connection-factory [& args]
  ;; load http2 module dynamically
  (let [http2-clear-connection-factory-ctor @(requiring-resolve 'ring.adapter.jetty9.http2/http2-clear-connection-factory)]
    (apply http2-clear-connection-factory-ctor args)))

(defn- https-connector [server http-configuration ssl-context-factory h2? h2-options port host max-idle-time]
  (let [secure-connection-factory (concat (when h2? (http2-alpn-connection-factory http-configuration h2-options))
...
(defn- http-connector [server http-configuration h2c? h2-options port host max-idle-time proxy?]
  (let [plain-connection-factories (cond-> [(HttpConnectionFactory. http-configuration)]
                                     h2c? (concat (http2-clear-connection-factory http-configuration h2-options))
...
(defn- quic-server-connector [& args]
  ...
  (let [quic-server-connector-ctor @(requiring-resolve 'ring.adapter.jetty9.http3/quic-server-connector)]
    (apply quic-server-connector-ctor args)))
